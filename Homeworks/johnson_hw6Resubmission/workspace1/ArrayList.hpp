...
void ArrayList<T>::mergeSort(int left, int right) {
	...
	if(left < right) {
		...
		//2 - call merge sort on front half
		mergeSort(left, middle);
		//3 - call merge sort on back half
		mergeSort(middle + 1, right);

		//4 - copy/sort elements from buffer to aux
		int a = left, b = middle + 1, c = 0;
		...
		//5 - copy sorted elements from aux back into buffer
		a = left;
		while(a <= right) {
			buffer[a] = aux[a - left];
			a++;
...
void ArrayList<T>::quicksort(int left, int right) {
	...
	if(left < right) {
		...
		{
			do x++;
			while(buffer[x] < buffer[right]);
			do y--;
			while(buffer[pivot] > buffer[right] && x < y);
			swap(x, pivot);
		}
		swap(x, right);

		//3 - recursively sort through sub-arrays
